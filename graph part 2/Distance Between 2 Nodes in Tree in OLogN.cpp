...
vector<int> v[10000],vis(10000,0);
const int maxn=50; // maxn=log2(n);
int level[10000],LCA[10000][maxn+1];

void dfs(int node,int lvl,int par){
    vis[node]=1;
    LCA[node][0]=par;
    for(auto x:v[node]){
        if(vis[x]==0)dfs(x,lvl+1,node);
    }
}
...
// distance between two nodes = level[a]+level[b]-2*level of LCA(a,b);
...
int main()
{
    ...
    // queries of finding distance between two given nodes
    int q;cin>>q;
    f(i,0,q){
        int a,b;cin>>a>>b;
        cs(distance(a,b));
